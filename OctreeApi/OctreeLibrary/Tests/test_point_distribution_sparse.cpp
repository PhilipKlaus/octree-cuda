        ...
        uint32_t index)
{
    ...
    if (octree[index].isFinished && octree[index].pointCount > 0)
    {
        ...
        {
            auto* point =
                    reinterpret_cast<Vector3<float>*> (cpuPointCloud + dataLUT[octree[index].chunkDataIndex + u] * 12);
    ...
    else
    {
        if (level > 0)
        {
            for (uint32_t i = 0; i < 8; ++i)
            {
                ...
                {
                    count += testOctreenodeSparse (
                            cpuPointCloud, octree, dataLUT, sparseToDense, level - 1, octree[index].childrenChunks[i]);
                }
            }
        }
    }
...
TEST_CASE ("Test point distributing sparse", "[distributing sparse]")
{
    ...
    REQUIRE (
            testOctreenodeSparse (cpuData.get (), octreeHost, dataLUT, sparseToDenseLUT, 7, topLevelIndex) ==
