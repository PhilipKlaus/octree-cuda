            ...
            if (octreeSparse[index].isParent)
            {
                if (lut[u] != INVALID_INDEX)
                {
                    ply.write (
                            ...
                            sizeof (float));
                    ply.write (
                            reinterpret_cast<const char*> (&(cpuPointCloud[lut[u] * metadata.pointDataStride + 4])),
                            sizeof (float));
                    ply.write (
...
void SparseOctree<coordinateType, colorType>::exportPlyNodes (const string& folderPath)
{
    ...
    uint32_t exportedPoints =
            exportTreeNode (cpuPointCloud.get (), octreeSparse, dataLUT, string ("r"), getRootIndex (), folderPath);
    assert (exportedPoints == itsMetadata.cloudMetadata.pointAmount);
    spdlog::info (
            "Sparse octree ({}/{} points) exported to: {}",
            exportedPoints,
