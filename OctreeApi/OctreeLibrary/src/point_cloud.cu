...
PointCloudHost::PointCloudHost (uint8_t* source, PointCloudMetadata metadata) :
        PointCloud(source, metadata)
{
...
PointCloudDevice::PointCloudDevice (uint8_t* source, PointCloudMetadata metadata) :
        PointCloud(source, metadata)
{}
...
uint8_t* PointCloudDevice::getCloudHost ()
{
    if(!itsHostCloud) {
        ...
        unique_ptr<uint8_t []> host (new uint8_t[cloudByteSize]);
        gpuErrchk (cudaMemcpy (host.get (), itsHostCloud.get(), sizeof (uint8_t) * cloudByteSize, cudaMemcpyDeviceToHost));
